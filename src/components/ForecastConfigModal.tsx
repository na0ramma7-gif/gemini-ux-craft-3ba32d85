// Legacy compatibility shim — kept so the Product-level forecast
// continues to work. The new Department Dashboard uses
// `ScenarioConfigModal` together with `lib/forecastEngine.ts`.
...
export const SCENARIO_DEFAULTS: Record<ScenarioType, Omit<ForecastConfig, 'horizon' | 'scenario'>> = {
...
const ForecastConfigModal = ({ open, onOpenChange, config, onApply }: Props) => {
  ...
  const handleScenarioChange = (scenario: ScenarioType) => {
    setLocal({ ...local, scenario, ...SCENARIO_DEFAULTS[scenario] });
  };

  const handleReset = () => {
    setLocal({ horizon: 12, scenario: 'baseline', ...SCENARIO_DEFAULTS['baseline'] });
  };
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-5 py-2">
              ...
              {(['baseline', 'optimistic', 'conservative'] as ScenarioType[]).map(s => (
                <button
                  ...
                  className={`flex-1 px-3 py-2 rounded-md text-xs font-medium transition-colors ${
                    local.scenario === s ? 'bg-card text-primary shadow-sm' : 'text-muted-foreground hover:text-foreground'
                  }`}
                >
                  {t(s as any)}
                </button>
          ...
          <div className="space-y-1.5">
            ...
            <Select value={local.horizon.toString()} onValueChange={(v) => setLocal({ ...local, horizon: Number(v) as HorizonMonths })}>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              ...
              <div className="relative">
                <Input type="number" step="0.5" min={-100} max={100}
        ...
        <DialogFooter className="gap-2">
          <Button variant="outline" size="sm" onClick={handleReset}>{t('resetDefaults')}</Button>
          <Button size="sm" onClick={() => { onApply(local); onOpenChange(false); }}>{t('applyConfig')}</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
